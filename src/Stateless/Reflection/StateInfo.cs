...
namespace Stateless.Reflection
{
    ...
    public class StateInfo
    {
        ...
        /// </summary>
        public IEnumerable<TransitionInfo> Transitions => this.FixedTransitions.Concat<TransitionInfo>(this.DynamicTransitions);
        ...
        internal static void AddRelationships<TState, TTrigger>(StateInfo info,
            StateMachine<TState, TTrigger>.StateRepresentation stateRepresentation, Func<TState, StateInfo> lookupState)
        {
            if (lookupState == null)
            {
            ...
            foreach (var triggerBehaviours in stateRepresentation.TriggerBehaviours)
            {
                ...
                foreach (var item in triggerBehaviours.Value.Where(behaviour =>
                    behaviour is StateMachine<TState, TTrigger>.TransitioningTriggerBehaviour))
                {
                    var destinationInfo =
                ...
                foreach (var item in triggerBehaviours.Value.Where(behaviour =>
                    behaviour is StateMachine<TState, TTrigger>.ReentryTriggerBehaviour))
                {
                    var destinationInfo =
                ...
                foreach (var item in triggerBehaviours.Value.Where(behaviour =>
                    behaviour is StateMachine<TState, TTrigger>.DynamicTriggerBehaviour))
                {
                    dynamicTransitions.Add(((StateMachine<TState, TTrigger>.DynamicTriggerBehaviour) item)
                        .TransitionInfo);
                }
            }
        ...
        {
            return this.UnderlyingState?.ToString() ?? "<null>";
        }
    }
}