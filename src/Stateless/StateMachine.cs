...
namespace Stateless
{
    ...
    {
        private readonly Queue<QueuedTrigger> eventQueue = new Queue<QueuedTrigger>();
        private readonly FiringMode firingMode;
        private readonly Func<TState> stateAccessor;

        private readonly IDictionary<TState, StateRepresentation> stateConfiguration =
            new Dictionary<TState, StateRepresentation>();

        private readonly Action<TState> stateMutator;

        private readonly IDictionary<TTrigger, TriggerWithParameters> triggerConfiguration =
            new Dictionary<TTrigger, TriggerWithParameters>();

        private bool firing;
        private readonly OnTransitionedEvent onTransitionedEvent;
        private UnhandledTriggerAction unhandledTriggerAction;
        ...
        public StateMachine(Func<TState> stateAccessor, Action<TState> stateMutator) : this(stateAccessor, stateMutator,
            FiringMode.Queued)
        {
        }
        ...
        {
            this.stateAccessor = stateAccessor ?? throw new ArgumentNullException(nameof(stateAccessor));
            this.stateMutator = stateMutator ?? throw new ArgumentNullException(nameof(stateMutator));
        ...
        public StateMachine(TState initialState, FiringMode firingMode) : this()
        {
            ...
            this.stateAccessor = () => reference.State;
            this.stateMutator = s => reference.State = s;
        ...
        {
            this.unhandledTriggerAction = new UnhandledTriggerAction.Sync(DefaultUnhandledTriggerAction);
            this.onTransitionedEvent = new OnTransitionedEvent();
        }
        ...
        public TState State
        {
            get => this.stateAccessor();
            private set => this.stateMutator(value);
        }
        ...
        private StateRepresentation CurrentRepresentation => GetRepresentation(this.State);
        ...
        {
            var representations = this.stateConfiguration.ToDictionary(kvp => kvp.Key, kvp => kvp.Value);

            var behaviours = this.stateConfiguration.SelectMany(kvp =>
                kvp.Value.TriggerBehaviours.SelectMany(b =>
                    b.Value.OfType<TransitioningTriggerBehaviour>().Select(tb => tb.Destination))).ToList();
            behaviours.AddRange(this.stateConfiguration.SelectMany(kvp =>
                kvp.Value.TriggerBehaviours.SelectMany(b =>
                    b.Value.OfType<ReentryTriggerBehaviour>().Select(tb => tb.Destination))).ToList());
        ...
        {
            if (!this.stateConfiguration.TryGetValue(state, out var result))
            {
                result = new StateRepresentation(state);
                this.stateConfiguration.Add(state, result);
            }
        ...
        ///     not allow the trigger to be fired.
        /// </exception>
        public void Fire<TArg0, TArg1, TArg2>(TriggerWithParameters<TArg0, TArg1, TArg2> trigger, TArg0 arg0,
        ...
        {
            var representativeState = GetRepresentation(this.State);
        ...
        private void InternalFireQueued(TTrigger trigger, params object[] args)
        {
            ...
            if (this.firing)
            {
                this.eventQueue.Enqueue(new QueuedTrigger {Trigger = trigger, Args = args});
                return;
            }
            ...
            try
            {
                ...
                {
                    var queuedEvent = this.eventQueue.Dequeue();
        ...
        private void InternalFireOne(TTrigger trigger, params object[] args)
        {
            ...
            if (this.triggerConfiguration.TryGetValue(trigger, out var configuration))
            {
            ...
            {
                this.unhandledTriggerAction.Execute(representativeState.UnderlyingState, trigger,
            ...
            switch (result.Handler)
            {
                ...
                case DynamicTriggerBehaviour _
                    when result.Handler.ResultsInTransitionFrom(source, args, out var destination):
                case TransitioningTriggerBehaviour _
                ...
                case InternalTriggerBehaviour _:
                {
                    ...
                    this.CurrentRepresentation.InternalAction(transition, args);
                    break;
                }
                default:
                    throw new InvalidOperationException(
                        "State machine configuration incorrect, no handler for trigger.");
            }
        }
        ...
        private void HandleReentryTrigger(object[] args, StateRepresentation representativeState, Transition transition)
        {
            ...
            this.onTransitionedEvent.Invoke(transition);

            newRepresentation.Enter(transition, args);
        }

        private void HandleTransitioningTrigger(object[] args, StateRepresentation representativeState,
            Transition transition)
        {
            ...
            if (newRepresentation.HasInitialTransition)
            {
                ...
                if (!newRepresentation.GetSubstates()
                    .Any(s => s.UnderlyingState.Equals(newRepresentation.InitialTransitionTarget)))
                {
                    throw new InvalidOperationException(
                        $"The target ({newRepresentation.InitialTransitionTarget}) for the initial transition is not a substate.");
                }
                ...
                {
                    var initialTransition = new Transition(transition.Source, newRepresentation.InitialTransitionTarget,
                        transition.Trigger);
        ...
        public void OnUnhandledTrigger(Action<TState, TTrigger> unhandledTriggerAction)
        {
            ...
            this.unhandledTriggerAction = new UnhandledTriggerAction.Sync((s, t, c) => unhandledTriggerAction(s, t));
        }
        ...
        public void OnUnhandledTrigger(Action<TState, TTrigger, ICollection<string>> unhandledTriggerAction)
        {
            ...
            this.unhandledTriggerAction = new UnhandledTriggerAction.Sync(unhandledTriggerAction);
        }
        ...
        public override string ToString()
        {
            return string.Format(
                "StateMachine {{ State = {0}, PermittedTriggers = {{ {1} }}}}", this.State,
        ...
        {
            if (this.triggerConfiguration.ContainsKey(trigger.Trigger))
            {
                throw new InvalidOperationException(
                    string.Format(StateMachineResources.CannotReconfigureParameters, trigger));
            }

            this.triggerConfiguration.Add(trigger.Trigger, trigger);
        }
