...
namespace Stateless
{
    public partial class StateMachine<TState, TTrigger>
    {
        ...
        private async Task InternalFireOneAsync(TTrigger trigger, params object[] args)
        {
            ...
            {
                await this.unhandledTriggerAction
            ...
            if (result.Handler is ReentryTriggerBehaviour handler)
            {
                ...
                await this.onTransitionedEvent.InvokeAsync(new Transition(source, handler.Destination, trigger));
            ...
            else if (result.Handler.ResultsInTransitionFrom(source, args, out var destination))
            {
                ...
                if (newRepresentation.HasInitialTransition)
                {
                    ...
                    if (!newRepresentation.GetSubstates().Any(s =>
                        s.UnderlyingState.Equals(newRepresentation.InitialTransitionTarget)))
                    {
                    ...
                    while (newRepresentation.GetSubstates().Any() && newRepresentation.HasInitialTransition)
                    {
                        var initialTransition =
                            new Transition(source, newRepresentation.InitialTransitionTarget, trigger);
                    ...
                    await this.onTransitionedEvent.InvokeAsync(new Transition(source, destination, trigger));
                }
                else
                {
                    //Alert all listeners of state transition
            ...
            else
            {
                ...
                await this.CurrentRepresentation.InternalActionAsync(transition, args).ConfigureAwait(false);
            }
        }
        ...
        public void OnUnhandledTriggerAsync(Func<TState, TTrigger, Task> unhandledTriggerAction)
        {
            ...
            this.unhandledTriggerAction = new UnhandledTriggerAction.Async((s, t, c) => unhandledTriggerAction(s, t));
        }
        ...
        public void OnUnhandledTriggerAsync(Func<TState, TTrigger, ICollection<string>, Task> unhandledTriggerAction)
        {
            ...
            this.unhandledTriggerAction = new UnhandledTriggerAction.Async(unhandledTriggerAction);
        }
